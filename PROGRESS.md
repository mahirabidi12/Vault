...
6. **Update, 2026-09-17: both packages are now actually published.** `pkgguard-mcp@0.1.0` is live under that exact name. The CLI is published as **`pkgguard-cli`**, not `pkgguard` — npm blocked the unscoped name as too similar to an unrelated existing package (`pkg-guard`). The command itself is still just `pkgguard` after install; only the npm package name differs. **Needs a doc fix:** the homepage/docs still show `npm install -g pkgguard`, which will fail — update to `npm install -g pkgguard-cli`.
