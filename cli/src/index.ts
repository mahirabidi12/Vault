...
program
  ...
  .action(async (packages: string[], opts: { yes?: boolean; json?: boolean; deep?: boolean }) => {
    process.exitCode = await runInstall(packages, { cwd: process.cwd(), yes: opts.yes, json: opts.json, deep: opts.deep });
  });
